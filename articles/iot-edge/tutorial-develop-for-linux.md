...
>[!NOTE]
>If you did not replace the **localhost:5000** value with the login server value from your Azure container registry, during the **["Create a project template"](#create-a-project-template)** step, this will cause the **.env** file to not be generated. The deployment manifest will also contain an empty registryCredentials section. 
